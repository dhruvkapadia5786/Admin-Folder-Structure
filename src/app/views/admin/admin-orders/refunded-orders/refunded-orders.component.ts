...
export class RefundedOrdersComponent implements OnInit {
  ...
  getRefundRequestedOrders(page:number,limit:number,sortBy:string='id',sortOrder:string='DESC',search:string=''){
    this.http.get<any>(`api/orders/list_refund_requested?page=${page}&limit=${limit}&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${search}`).subscribe((resp) => {
  ...
  async _getPaymentDetailsForOrder(orderId:any){
    const url = `api/orders/charge_details/${orderId}`;
