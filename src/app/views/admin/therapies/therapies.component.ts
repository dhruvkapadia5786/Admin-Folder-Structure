...
export class TherapiesComponent implements OnInit,AfterViewInit,OnDestroy {
  therapiesTableData = [];
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this._http
          ...
          .subscribe((resp:any) => {
            this.therapiesTableData = resp.data;
      ...
      columns: [
        ...
        },
        {
          ...
          render: function (data: any, type: any, full: any) {
            return `<button class="btn btn-sm btn-primary m-0" therapyEditId=${full._id}>Edit</button>`;
          },
          orderable: false
        }
      ]
  ...
  ngAfterViewInit(): void {
    ...
    this.listenerFn = this._renderer.listen('document', 'click', (event:any) => {
      if (event.target.hasAttribute('therapyEditId')) {
        this.openEditModal(event.target.getAttribute('therapyEditId'));
      }
    });
  }
  ...
  openAddModal(){
    this._therapyAddEditModalService.setData({event:'ADD'})
    this.modalRef = this.modalService.show(TherapyAddEditModalComponent);
  ...
  openEditModal(id:any){
    let data = this.therapiesTableData.find((item:any)=>item._id == id);
    this._therapyAddEditModalService.setData({event:'EDIT',data:data});
    this.modalRef = this.modalService.show(TherapyAddEditModalComponent);
    this.modalRef.content.onEventCompleted.subscribe(()=>{
      this.rerender();
    });
