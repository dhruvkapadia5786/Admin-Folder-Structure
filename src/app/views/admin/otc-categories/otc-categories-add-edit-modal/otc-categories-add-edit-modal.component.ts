...
@Component({
  selector: 'app-otc-categories-add-edit-modal',
  templateUrl: './otc-categories-add-edit-modal.component.html',
  styleUrls: ['./otc-categories-add-edit-modal.component.scss']
})
export class OtcCategoriesAddEditModalComponent implements OnInit {
  ...
  constructor(
    ...
    private _tcAddEditModalService: OtcCategoriesAddEditModalService
  ) {
  ...
  async saveTreatmentCondition(formValid:boolean){
    if(formValid){
      ...
      formData.append('otc_category', JSON.stringify(this.treatmentConditionForm.value));
      formData.append('image_url', this.selectedImageFile);

      if(this.modalEvent == 'ADD') {
        let create = await  this._tcAddEditModalService.addNewOtcCategories(formData);
      } else if (this.modalEvent == 'EDIT') {
        let update = await  this._tcAddEditModalService.editOtcCategories(this.treatmentConditionForm.value.id,formData);
      }
